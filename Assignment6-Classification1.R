...
# [Ans:]
# Cost of sub-tree = Misclassification rate + cp*(number of leaves in sub-tree)
# Need to minimize this cost for various cp values. Need to collapse leaves in a bottom-up fashion
# until the cost reduction by minimizing the number of leaves remains greater than the resulting
# increase in the misclassification rate

# cp = 0.01
# =========
# Initially, misclassification rate = 0, num of leaves = 7
# Cost = 0 + 0.01*7 = 0.07
# 
# Trying to collapse "root>age=youth,senior>student=no,youth=senior".
# Misclassification rate = 1/14 (since 1 entry will be misclassified), num leaves = 6 (since 2 leaves replaced with 1 leaf)
# Cost = 1/14 + 0.01*6 = 0.1314
# The cost becomes greater than the initial cost; hence cannot collapse.
# Similarly, we cannot collapse "root>age=youth,senior,student=yes,credit=excellent".
# 
# So, the tree remains unchanged during post-pruning for cp = 0.01. Verifiable from below rpart() call.
rpart(buy ~ age + income + student + credit.rating, computer,control=rpart.control(cp=0.01,minsplit=2))

# cp = 0.05
# =========
# The leaves cannot be collapsed even now. This is because the increased misclassification rate of
# 1/14 is greater than the 0.05 reduction due to collapsing the leaves.
# So, tree remains unchanged. Verifiable from below rpart() call.
rpart(buy ~ age + income + student + credit.rating, computer,control=rpart.control(cp=0.05,minsplit=2))

# cp = 0.08
# =========
# The leaves can be collapsed now. This is because the increased misclassification rate of 1/14
# is lesser than the 0.08 reduction due to collapsing the leaves. (0,1) + (1,0) -> (1,1). So, the nodes
# "root>age=youth,senior>student=no,youth=senior" and
# "root>age=youth,senior,student=yes,credit=excellent" get collapsed.

# Also, we can further collapse "root>age=youth,senior>student=no" and
# "root>age=youth,senior>student=yes" as they do NOT increase the misclassification rate.
# [A] (1,1) + (3,0) -> (4,1)  [B] (0,3) + (1,1) -> (1,4) ; there is still only 1 mis-classification

# But we CANNOT collapse (4,1) + (1,4) to (5,5) as it will cause 3 more (=5-1-1) mis-classifications,
# and 3/14 > 0.08

# WHY DOES THIS NOT CAUSE THE EXPECTED POST-PRUNING?
rpart(buy ~ age + income + student + credit.rating, computer,control=rpart.control(cp=0.08,minsplit=2))
# THE EXPECTED POST-PRUNING HAPPENS ONLY WHEN CP IS INCREASED TO 0.1
rpart(buy ~ age + income + student + credit.rating, computer,control=rpart.control(cp=0.2999,minsplit=2))
...
#########################################################################
# Problem 2: Applying Naïve Bayes Algorithm on Continuous & Categorical Data
